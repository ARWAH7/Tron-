...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const checkMemory = () => {
      ...
      if (performance.memory) {
        ...
        // 自动清理机制：根据内存使用情况分级清理（优化后：更早触发清理）
        if (percentage > 85) {
          ...
          setAllBlocks(prev => {
            if (prev.length > 3000) {
              console.log(`[内存] 紧急清理区块数据: ${prev.length} → 3000`);
              return prev.slice(0, 3000);
            }
            return prev;
          });
        } else if (percentage > 75) {
          console.warn(`[内存] 使用率 ${percentage}%，触发自动清理`);
          
          // 常规清理：保留最近 5,000 个区块
          setAllBlocks(prev => {
            if (prev.length > 5000) {
              console.log(`[内存] 清理区块数据: ${prev.length} → 5000`);
              return prev.slice(0, 5000);
            }
            return prev;
          });
          ...
          setAllBlocks(prev => {
            if (prev.length > 7000) {
              console.log(`[内存] 轻度清理区块数据: ${prev.length} → 7000`);
              return prev.slice(0, 7000);
            }
            return prev;
          });
        }
  ...
  const requiredDataCount = useMemo(() => {
    ...
    // 珠盘路需要: 行数 × 列数 = beadRows × 44
    // 确保只加载必要的数据量
    const beadRequired = (activeRule.beadRows || 6) * 44;
    
    console.log(`[数据需求] 规则: ${activeRule.label}, 珠盘路: ${beadRequired} 条`);
  ...
  const ruleFilteredBlocks = useMemo(() => {
    ...
    // 只返回符合当前规则的前 requiredDataCount 条数据
    const limited = filtered.slice(0, requiredDataCount);
    const endTime = performance.now();
    
    // 只在开发模式下输出性能日志
    if (process.env.NODE_ENV === 'development') {
      console.log(`[过滤] 规则: ${activeRule.label} (步长${activeRule.value}), 总区块: ${allBlocks.length}, 过滤后: ${filtered.length}, 限制后: ${limited.length}, 耗时: ${(endTime - startTime).toFixed(2)}ms`);
    }
    
    return limited;
  }, [allBlocks, activeRule, checkAlignment, requiredDataCount]);
  ...
  // 从后端 API 加载历史数据的函数（根据规则动态计算所需数据量）
  const loadHistoryBlocks = useCallback(async (forceReload: boolean = false) => {
    try {
      ...
      // 精确计算需要加载的原始数据量，确保筛选后有足够的数据
      // 考虑到可能的区块缺失，添加适当的缓冲
      const estimatedRaw = Math.ceil(requiredFiltered * ruleValue * 1.2);
      const limitToLoad = Math.min(estimatedRaw, 30000);
      
      // 检查当前数据是否足够（避免不必要的重新加载）
      if (!forceReload && allBlocks.length >= limitToLoad * 0.9) {
        console.log(`[API] 数据已足够 (当前: ${allBlocks.length}, 需要: ${limitToLoad})，跳过加载`);
        setIsLoading(false);
        return;
      }
      ...
      console.log(`[API] 正在从 Redis 加载历史数据...`);
      console.log(`[API] 规则步长: ${ruleValue}, 需要过滤后: ${requiredFiltered} 条, 加载原始: ${limitToLoad} 条`);
      
      const BACKEND_API_URL = 'http://localhost:3001';
      const response = await fetch(`${BACKEND_API_URL}/api/blocks?limit=${limitToLoad}`);
      const result = await response.json();
      
      if (result.success) {
        setAllBlocks(result.data);
        console.log(`[API] ✅ 从 Redis 加载 ${result.data.length} 个历史区块`);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // 只在数据不足时才加载
      if (allBlocks.length < limitToLoad * 0.9) {
        console.log(`[规则变化] 切换到规则: ${activeRule.label}, 数据不足，需要加载`);
        loadHistoryBlocks(false);
      } else {
        console.log(`[规则变化] 切换到规则: ${activeRule.label}, 数据已足够，无需加载`);
      }
    }, 300); // 300ms 防抖
  ...
  useEffect(() => {
    const BACKEND_WS_URL = import.meta.env.VITE_WS_BASE_URL || 'ws://localhost:8080';
        ...
        ws.onmessage = (event) => {
          try {
            ...
            setAllBlocks(prev => {
              ...
              // 添加新区块并排序，限制最大数量为 30000（后端限制）
              const MAX_BLOCKS_FRONTEND = 30000;
    ...
    setTimeout(() => {
      if (!wsConnected) {
        console.log('[连接] WebSocket 连接失败，尝试从 API 加载历史数据');
        loadHistoryBlocks(true);
      }
    }, 3000);
